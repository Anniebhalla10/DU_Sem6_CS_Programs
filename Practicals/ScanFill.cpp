// Scan fill algorithm to fill a polygon
// Annie Bhalla
// 19HCS4009
...
void poly::read()
{
    ...
    if(v>2)
    {
        ...
        //last coordinates will be equal to the first pair
        p[i].x=p[0].x;
        p[i].y=p[0].y;
...
void poly::calcs()
{ //to set values of xmax xmin, ymax, ymin
    for(int i=0;i<v;i++)
    {
        if( xmin > p[i].x )
        	xmin = p[i].x;
...
void poly::intersections(float z) //DEFINE FUNCTION intersections
{
