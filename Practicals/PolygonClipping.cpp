// Polyogn Clipping - Sutherland Hodgeman
// Annie Bhalla
// 19HCS4009
...
void drawpoly(vertex vlist[], int n){
	 for(int i=0; i<n; i++)
        line(vlist[i].x, vlist[i].y, vlist[(i+1)%n].x, vlist[(i+1)%n].y);  // mod n taki vo vapis index 0 pe ajaye after end index
}
...
int main(){
    ...
    cin>>n_cw;
    cout<<"Enter vertices (x,y) clockwise of clipping window"<<endl;
    ...
    char ch;
    cout<<"Press any key to clip from an edge of clipping window"<<endl;
    for(int i=0; i<n_cw; i++)  // har edge se clipping krungi , so it will go upto number of clipping window vertices
    {
        cin>>ch;
